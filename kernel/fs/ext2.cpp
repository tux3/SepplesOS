...
{
    int i, j;
    struct ext2Disk* hd = new struct ext2Disk;
    hd->device = device;
    hd->part = part;
    hd->sb = ext2ReadSb(hd, (u64)part.sLba * 512);
...
struct ext2SuperBlock *ext2ReadSb(struct ext2Disk *hd, u64 sPart)
{
    struct ext2SuperBlock* sb = new struct ext2SuperBlock;
    diskRead(hd->device, (u64)sPart + 1024, (char *) sb, sizeof(struct ext2SuperBlock));
    return sb;
}

struct ext2GroupDesc *ext2ReadGd(struct ext2Disk *hd, u64 sPart)
{
    struct ext2GroupDesc *gd;
    u64 offset, gdSize;

    offset = (hd->blocksize == 1024) ? 2048 : hd->blocksize; // locate block
    gdSize = hd->groups * sizeof(struct ext2GroupDesc); // size occupied by the descriptors
...
struct ext2Inode *ext2ReadInode(const struct ext2Disk *hd, const int iNum)
{
    ...
    // Read
    u64 readPos = ((u64)hd->part.sLba * 512) + offset;
    diskRead(hd->device, readPos, (char*) ext2Inode, sizeof(struct ext2Inode));
    return ext2Inode;
}
...
char *ext2ReadFile(const struct ext2Disk *hd, const struct ext2Inode *ext2Inode)
{
    ...
    {
        diskRead(hd->device, baseOff + ext2Inode->iBlock[12] * (u64)hd->blocksize, (char *) p, hd->blocksize);
        for (i = 0; i < hd->blocksize / 4 && p[i]; i++) {
            diskRead(hd->device, baseOff + p[i] * hd->blocksize, buf, hd->blocksize);
    ...
    {
        diskRead(hd->device, baseOff + ext2Inode->iBlock[13]*hd->blocksize, (char *) p, hd->blocksize);

        for (i = 0; i < hd->blocksize / 4 && p[i]; i++) {
            diskRead(hd->device, baseOff + p[i]*hd->blocksize, (char *) pp, hd->blocksize);

            for (j = 0; j < hd->blocksize / 4 && pp[j]; j++) {
                diskRead(hd->device, baseOff + pp[j]*hd->blocksize, buf, hd->blocksize);
    ...
    if (ext2Inode->iBlock[14]) {
        diskRead(hd->device, ((u64)hd->part.sLba * 512) + ext2Inode->iBlock[14] * hd->blocksize, (char *) p, hd->blocksize);

        for (i = 0; i < hd->blocksize / 4 && p[i]; i++) {
            diskRead(hd->device, ((u64)hd->part.sLba * 512) + p[i] * hd->blocksize, (char *) pp, hd->blocksize);

            for (j = 0; j < hd->blocksize / 4 && pp[j]; j++) {
                diskRead(hd->device, ((u64)hd->part.sLba * 512) + pp[j] * hd->blocksize, (char *) ppp, hd->blocksize);

                for (k = 0; k < hd->blocksize / 4 && ppp[k]; k++) {
                    diskRead(hd->device, ((u64)hd->part.sLba * 512) + ppp[k] * hd->blocksize, buf, hd->blocksize);
...
//				ext2Inode = ext2_read_inode(hd, inum);
    ...
    while (dentry->ext2Inode && dsize)
    {
        ...
        //gTerm.printf("NodeEXT2::updateChildsCache(): recLen 0x%x\n", dentry->recLen);
        //gTerm.printf("NodeEXT2::updateChildsCache(): reading inode %d: %s\n", dentry->ext2Inode, filename);
